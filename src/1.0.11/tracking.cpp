...
{
    timeval tim;
    double get_im_t, preproc_t, track_t, etalon_update_t, output_t;
...
bool contrast_tracking(Tracking *track, float *image, window *track_window, bool *first_frame, int tracking_mode, bool debug_mode)
{
    ...
    timeval tim;
    double start_t, get_im_t, preproc_t, track_t, output_t;
    int last_target_size = track->target_size;
	...
	if(*first_frame == true)
	{
        ...
        *track_window = track->getOutput(image, 0);
        ...
        {
            if((int)contours[i].size() > track->target_size )
            {
    ...
    if(track->target_size > 30 && last_target_size > 30)
    {
    	*first_frame = true;
    	return false;
    }
